...
void clearROI(const Data &data) {
    auto roi = data.camera->get_device().get_facility<Metavision::I_ROI>();
    roi->enable(false);
}

void applyROI(const Data &data) {
    bool ret    = false;
    auto roi_px = data.camera->get_device().get_facility<Metavision::I_RoiPixelMask>();
    if (roi_px) {
        if (!data.mask.empty()) {
        ...
        while (!signal_caught && camera.is_running()) {
            ...
            if (events.size() > static_cast<size_t>(min_event_count) &&
                ...
                if (!calib_setup) {
                    ...
                    cv::resizeWindow(calib_window_name, geometry.get_width(), geometry.get_height());
                    cv::moveWindow(calib_window_name, 0, geometry.get_height());
#if (CV_MAJOR_VERSION == 3 && (CV_MINOR_VERSION * 100 + CV_SUBMINOR_VERSION) >= 408) || \
    (CV_MAJOR_VERSION == 4 && (CV_MINOR_VERSION * 100 + CV_SUBMINOR_VERSION) >= 102)
